#!/usr/bin/env bash
# new_leg.sh — scaffold a new dated experiment leg from the current leader
# Usage: bash scripts/new_leg.sh <name>
#   name:  short hypothesis name (no spaces, use underscores)
# Example: bash scripts/new_leg.sh BW10_anchor
set -euo pipefail
...
[[ -n "${NAME}" ]]  || { echo "Usage: bash scripts/new_leg.sh <name>"; exit 1; }

TODAY="$(date +%Y-%m-%d)"
LEG_DIR="${REPO_ROOT}/legs/${TODAY}_${NAME}"

[[ ! -d "${LEG_DIR}" ]] || { echo "Already exists: ${LEG_DIR}"; exit 1; }
mkdir -p "${LEG_DIR}"

# Copy leader's train_gpt.py as starting point
LEADER_LEG=$(grep "^Leg:" "${REPO_ROOT}/LEADER.md" | awk '{print $2}')

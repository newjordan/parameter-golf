...
def _parse_float_tuple(key: str, default: str) -> tuple[float, ...]:
    vals = tuple(float(x) for x in os.environ.get(key, default).split(",") if x.strip())
    if not vals:
        raise ValueError(f"{key} must have at least one comma-separated float")
    return vals
class Hyperparameters:
    ...
    # Crawler Katta: fast Runge-Kutta-style loop solver.
    # euler: baseline single crawler pass per loop.
    # rk2_fast/rk4_fast/rk24_hybrid: one real pass + lightweight per-head mixers for intermediate stages.
    crawler_solver = os.environ.get("CRAWLER_SOLVER", "euler").strip().lower()
    crawler_rk_fast_heads = int(os.environ.get("CRAWLER_RK_FAST_HEADS", "2"))
    crawler_rk_blend_init = float(os.environ.get("CRAWLER_RK_BLEND_INIT", "-4.0"))
    crawler_rk_recur_gain_init = float(os.environ.get("CRAWLER_RK_RECUR_GAIN_INIT", "0.0"))
    crawler_rk_hybrid_mix_init = float(os.environ.get("CRAWLER_RK_HYBRID_MIX_INIT", "-1.5"))
    crawler_rk_battery = _parse_float_tuple("CRAWLER_RK_BATTERY", "1.0,1.0,1.0,1.0")
...
class LoopSmearGate(nn.Module):
    ...
    def forward(self, x: Tensor, x_prev: Tensor) -> Tensor:
        g = torch.sigmoid(self.gate.to(dtype=x.dtype))[None, None, :]
        return (1 - g) * x + g * x_prev
class RKFastHeadMixer(nn.Module):
    """Low-cost per-head projector used to approximate RK intermediate stages."""
    def __init__(self, model_dim: int, num_heads: int, active_heads: int):
        super().__init__()
        ...
        self.active_heads = max(1, min(active_heads, num_heads))
        self.active_dim = self.active_heads * self.head_dim
        self.proj = CastedLinear(self.active_dim, self.active_dim, bias=False)
        self.proj._zero_init = True
        self.head_gain = nn.Parameter(torch.zeros(self.active_heads, dtype=torch.float32))
    def forward(self, x: Tensor) -> Tensor:
        bsz, seqlen, _ = x.shape
        xh = x.reshape(bsz, seqlen, self.num_heads, self.head_dim)
        active = xh[:, :, : self.active_heads, :].reshape(bsz, seqlen, self.active_dim)
        delta = self.proj(active).reshape(bsz, seqlen, self.active_heads, self.head_dim)
        gain = torch.tanh(self.head_gain.to(dtype=x.dtype))[None, None, :, None]
        delta = (delta * gain).reshape(bsz, seqlen, self.active_dim)
        if self.active_dim == self.model_dim:
            return delta
        return F.pad(delta, (0, self.model_dim - self.active_dim))
...
class CrawlerGPT(nn.Module):
    ...
    def __init__(
        ...
        crawler_solver: str = "euler",
        crawler_rk_fast_heads: int = 2,
        crawler_rk_blend_init: float = -4.0,
        crawler_rk_recur_gain_init: float = 0.0,
        crawler_rk_hybrid_mix_init: float = -1.5,
        crawler_rk_battery: tuple[float, ...] = (1.0, 1.0, 1.0, 1.0),
    ):
        super().__init__()
        ...
        self.crawler_solver = crawler_solver.lower()
        if self.crawler_solver not in {"euler", "rk2_fast", "rk4_fast", "rk24_hybrid"}:
            raise ValueError(
                f"Unsupported CRAWLER_SOLVER '{crawler_solver}'. Use euler/rk2_fast/rk4_fast/rk24_hybrid."
            )
        self.crawler_rk_battery = crawler_rk_battery if len(crawler_rk_battery) > 0 else (1.0, 1.0, 1.0, 1.0)
        # Compatibility stubs
        ...
        if num_crawler_layers > 0 and crawler_loops > 1 and inst_dim > 0:
            ...
            self.loop_inst_up = None
        self.delta_net = None
        self.rk_fast_heads = 0
        if num_crawler_layers > 0 and self.crawler_solver != "euler":
            self.rk_fast_heads = max(1, min(crawler_rk_fast_heads, num_heads))
            self.rk_mix_k2 = RKFastHeadMixer(model_dim, num_heads, self.rk_fast_heads)
            self.rk_mix_k3 = (
                RKFastHeadMixer(model_dim, num_heads, self.rk_fast_heads)
                if self.crawler_solver in {"rk4_fast", "rk24_hybrid"}
                else None
            )
            self.rk_mix_k4 = (
                RKFastHeadMixer(model_dim, num_heads, self.rk_fast_heads)
            ...
            self.rk_blend = nn.Parameter(torch.tensor(crawler_rk_blend_init, dtype=torch.float32))
            self.rk_recur_gain = nn.Parameter(torch.tensor(crawler_rk_recur_gain_init, dtype=torch.float32))
            self.rk_hybrid_mix = (
                nn.Parameter(torch.tensor(crawler_rk_hybrid_mix_init, dtype=torch.float32))
                if self.crawler_solver == "rk24_hybrid"
                else None
            )
        else:
            self.rk_mix_k2 = None
            ...
            self.rk_mix_k4 = None
            self.rk_blend = None
            self.rk_recur_gain = None
            self.rk_hybrid_mix = None
    ...
    def _rk_b(self, idx: int) -> float:
        if idx < len(self.crawler_rk_battery):
            return float(self.crawler_rk_battery[idx])
        return float(self.crawler_rk_battery[-1])

    def _run_crawler_stack(
        self,
        x_loop: Tensor,
        x0: Tensor,
        input_ids: Tensor,
    ...
    def _run_crawler_solver(
        ...
        loop: int,
        lcs: tuple | None,
        rk_prev_delta: Tensor | None,
    ) -> tuple[Tensor, Tensor | None]:
        x_euler = self._run_crawler_stack(x_in, x0, input_ids, ve_cache, loop, lcs)
        if self.rk_mix_k2 is None:
            return x_euler, None

        # Fast RK approximation:
        # - one full crawler stack (d1),
        # - intermediate stages predicted by cheap per-head mixers,
        # - blended back toward Euler for stability/warm start.
        d1 = x_euler - x_in
        recur = 0.0
        if rk_prev_delta is not None and self.rk_recur_gain is not None:
            recur = torch.tanh(self.rk_recur_gain.to(dtype=x_in.dtype)) * rk_prev_delta
        b1 = self._rk_b(0)
        b2 = self._rk_b(1)
        d2 = self.rk_mix_k2(x_in + 0.5 * b1 * d1 + recur) * b2

        if self.crawler_solver in {"rk4_fast", "rk24_hybrid"} and self.rk_mix_k3 is not None and self.rk_mix_k4 is not None:
            d3 = self.rk_mix_k3(x_in + 0.5 * d2 + recur) * self._rk_b(2)
            d4 = self.rk_mix_k4(x_in + d3 + recur) * self._rk_b(3)
            x_rk4 = x_in + (b1 * d1 + 2.0 * d2 + 2.0 * d3 + d4) / 6.0
            if self.crawler_solver == "rk24_hybrid" and self.rk_hybrid_mix is not None:
                x_rk2 = x_in + d2
                hybrid_mix = torch.sigmoid(self.rk_hybrid_mix.to(dtype=x_in.dtype))
                x_rk = x_rk2 + hybrid_mix * (x_rk4 - x_rk2)
            else:
                x_rk = x_rk4
        else:
            x_rk = x_in + d2

        blend = torch.sigmoid(self.rk_blend.to(dtype=x_in.dtype)) if self.rk_blend is not None else x_in.new_tensor(0.0)
        x_out = x_euler + blend * (x_rk - x_euler)
        return x_out, (x_out - x_in)
    ...
    def _run_crawler(self, x: Tensor, x0: Tensor, input_ids: Tensor, ve_cache: dict,
        ...
        rk_prev_delta: Tensor | None = None
        ...
        for loop in range(self.crawler_loops):
            ...
            x_loop, rk_prev_delta = self._run_crawler_solver(
                x_loop, x0, input_ids, ve_cache, loop, lcs, rk_prev_delta
            )
...
def _get_block_named_params(model: nn.Module) -> list:
    ...
    if isinstance(model, CrawlerGPT):
        named = list(model.flat_blocks.named_parameters()) + list(model.crawler_blocks.named_parameters())
        if model.rk_mix_k2 is not None:
            named += [(f"rk_mix_k2.{name}", p) for name, p in model.rk_mix_k2.named_parameters()]
            if model.rk_mix_k3 is not None:
                named += [(f"rk_mix_k3.{name}", p) for name, p in model.rk_mix_k3.named_parameters()]
            if model.rk_mix_k4 is not None:
                named += [(f"rk_mix_k4.{name}", p) for name, p in model.rk_mix_k4.named_parameters()]
            if model.rk_blend is not None:
                named.append(("rk_blend", model.rk_blend))
            if model.rk_recur_gain is not None:
                named.append(("rk_recur_gain", model.rk_recur_gain))
            if model.rk_hybrid_mix is not None:
                named.append(("rk_hybrid_mix", model.rk_hybrid_mix))
        return named
...
def build_model(args: Hyperparameters, device: torch.device) -> nn.Module:
    ...
    if args.use_crawler:
        model = CrawlerGPT(
            ...
            crawler_solver=args.crawler_solver,
            crawler_rk_fast_heads=args.crawler_rk_fast_heads,
            crawler_rk_blend_init=args.crawler_rk_blend_init,
            crawler_rk_recur_gain_init=args.crawler_rk_recur_gain_init,
            crawler_rk_hybrid_mix_init=args.crawler_rk_hybrid_mix_init,
            crawler_rk_battery=args.crawler_rk_battery,
        )
    else:
        model = GPT(
            vocab_size=args.vocab_size,
...
def main() -> None:
    ...
    log0(f"crawler_solver:{args.crawler_solver} rk_fast_heads:{args.crawler_rk_fast_heads} rk_blend_init:{args.crawler_rk_blend_init} rk_recur_gain_init:{args.crawler_rk_recur_gain_init} rk_hybrid_mix_init:{args.crawler_rk_hybrid_mix_init} rk_battery:{args.crawler_rk_battery}")

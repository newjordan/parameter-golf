...
class Hyperparameters:
    ...
    ttt_freeze_first_n = int(os.environ.get("TTT_FREEZE_FIRST_N", 0))
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_delta=None, v_delta=None) -> Tensor:
        ...
        # XSA: Exclusive Self Attention — subtract self-attention projection (arXiv:2603.09078)
        # FA3 output layout: (B, T, H, D) — reshape into GQA groups on H dim
        if self.use_xsa:
            ...
            y = (y_grouped - dot * vn).reshape(bsz, seqlen, self.num_heads, self.head_dim)
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, lora=None) -> Tensor:
        ...
        if lora:
            # TTT mode: return per-token losses for chunked training
...
def eval_val_ttt_lora(
    ...
    for bi in range(0, len(rank_docs), batch_size):
        ...
        # Multi-epoch TTT: (ttt_epochs - 1) training-only passes

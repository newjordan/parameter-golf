...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if _USE_FA3:
            ...
            # Fallback: PyTorch SDP (flash backend). Needs (bsz, heads, seq, dim) layout.
            q2 = q.transpose(1, 2)
            k2 = k.transpose(1, 2)
            v2 = v.transpose(1, 2)
...
def main() -> None:
    ...
    mlp_schedule: list[float] | None = None
    if args.mlp_schedule_str:
        mlp_schedule = [float(x.strip()) for x in args.mlp_schedule_str.split(",")]
        if len(mlp_schedule) != args.num_layers:

...
class Hyperparameters:
    ...
    loop_adapter_rank = int(os.environ.get("LOOP_ADAPTER_RANK", "0"))         # per-loop LoRA rank on crawler MLP fc (0=off); breaks shared-weight fixed-point collapse
    ...
    compile_mode = os.environ.get("COMPILE_MODE", "") or None  # e.g. "max-autotune"; None = torch default
...
def maybe_torch_compile(obj, args: Hyperparameters):
    if not args.compile_enabled:
        return obj
    return torch.compile(obj, dynamic=False, fullgraph=args.compile_fullgraph, mode=args.compile_mode)
...
class MLP(nn.Module):
    ...
    def forward(self, x: Tensor, loop_adapter=None) -> Tensor:
        h = self.fc(x)
        if loop_adapter is not None:
            b_proj, a_proj = loop_adapter[0], loop_adapter[1]
            h = h + a_proj(b_proj(x))
        if self.mlp_act == "leaky_relu_sq":
            h = F.leaky_relu(h, negative_slope=self.mlp_leaky_slope)
        else:
            h = F.relu(h)
...
class Block(nn.Module):
    def __init__(
        ...
        if dtg:
            ...
            self.dtg_gate = None
    def forward(self, x: Tensor, x0: Tensor, v_embed: Tensor | None = None, loop_adapter=None) -> Tensor:
        mix = self.resid_mix.to(dtype=x.dtype)
        x_in = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        attn_out = self.attn(self.attn_norm(x_in) * self.ln_scale_factor, v_embed=v_embed)
        x_out = x_in + self.attn_scale.to(dtype=x_in.dtype)[None, None, :] * attn_out
        x_out = x_out + self.mlp_scale.to(dtype=x_out.dtype)[None, None, :] * self.mlp(self.mlp_norm(x_out) * self.ln_scale_factor, loop_adapter=loop_adapter)
...
class CrawlerGPT(nn.Module):
    ...
    def __init__(
        ...
        if delta_net_heads > 0 and num_crawler_layers > 0:
            ...
            self.delta_net = None
        # Per-loop LoRA adapters on crawler MLP fc — breaks shared-weight fixed-point collapse.
        # Each loop k gets unique A_k @ B_k correction: h = W_fc @ x + A_k @ (B_k @ x)
        # Zero-init A_k so adapters start as identity; B_k gets kaiming init.
        # Artifact cost: K * 2 * rank * (dim + hidden) params — negligible at rank<=4.
        if loop_adapter_rank > 0 and num_crawler_layers > 0:
            hidden = int(crawler_mlp_mult * model_dim)
            adapter_list = []
            for _ in range(crawler_loops):
                b_proj = CastedLinear(model_dim, loop_adapter_rank, bias=False)
                a_proj = CastedLinear(loop_adapter_rank, hidden, bias=False)
                a_proj._zero_init = True  # ensures _init_weights zeroes it out
                adapter_list.append(nn.ModuleList([b_proj, a_proj]))
            self.loop_fc_adapters = nn.ModuleList(adapter_list)
        else:
            self.loop_fc_adapters = None
    ...
    def _run_crawler(self, x: Tensor, x0: Tensor, input_ids: Tensor, ve_cache: dict) -> Tensor:
        ...
        for loop in range(self.crawler_loops):
            ...
            for ci, block in enumerate(self.crawler_blocks):
                ...
                adapter = self.loop_fc_adapters[loop] if self.loop_fc_adapters is not None else None
                x_loop = block(x_loop, x0, v_embed=ve, loop_adapter=adapter)

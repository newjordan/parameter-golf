#!/bin/bash
set -euo pipefail
# crawler_looped: per-loop LoRA adapters on crawler MLP fc
#
# Hypothesis: shared-weight fixed-point collapse is broken by giving each
# loop k a unique low-rank correction A_k @ B_k on the MLP up-projection.
# Each loop's gradient flows only through its own adapter — unique optimization
# target per loop — preventing convergence to a single shared fixed point.
#
# LOOP_ADAPTER_RANK=0  →  identical to Crawler_Leg_1 (baseline / collapse)
# LOOP_ADAPTER_RANK=1  →  10K params total, minimal artifact cost
# LOOP_ADAPTER_RANK=4  →  40K params (recommended first real test)
#
# All other vars locked to Crawler_Leg_1 for clean comparison.
...
# Locked to Crawler_Leg_1 — do not change for A/B
NUM_FLAT_LAYERS=4
...
LOOP_ADAPTER_RANK="${LOOP_ADAPTER_RANK:-4}"
...
echo "  LOOP_ADAPTER_RANK=${LOOP_ADAPTER_RANK}"
...
LOOP_ADAPTER_RANK="${LOOP_ADAPTER_RANK}" \
DELTA_NET_HEADS=0 \
SKIP_EMA=1 \
SKIP_GPTQ=1 \
LOOP_AWARE_GPTQ=0 \
...
torchrun --standalone --nproc_per_node="${NPROC_PER_NODE}" \
    ...
    2>&1 | tee "logs/crawler_looped_r${LOOP_ADAPTER_RANK}_s${SEED}_$(date +%Y%m%d_%H%M%S).log"

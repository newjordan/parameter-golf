#!/bin/bash
set -euo pipefail
# BANDIT_II: Bandit (crawler + X-WING oracle) with confirmed LOOP_AWARE_GPTQ
#
# What changed from Bandit I:
#   LOOP_AWARE_GPTQ=1  — confirmed by Crawler_Ablations_v1 arm B
#   (arm B: +60 steps in 600s, val_bpb -0.017 vs one-shot GPTQ baseline)
#
# Legality:
#   GPTQ calibration uses train_pattern (training data) ONLY.
#   Calibration fires after stopping_early (wallclock cap) — never during eval.
#   See: experiments/Bandit/train_gpt.py gptq_calibrate() line ~2566.
#   SCORE-FIRST protocol satisfied: val data never used before scoring.
#
# Architecture: same as Bandit I
#   4 flat + 1 crawler x 4 loops | inst_dim=32 FLOW | DN=0 (quarantined)
#   EMA_START_STEP=4400 | EMA_DECAY=0.99
#   NGRAM_EVAL_ORDER=9 | CUBRIC_CADENCE=32 | COMPLEMENT_ALPHA=0.5
#   Shared n-gram tables | 3D Cubric 54-cell warm-start
#
# Train script: experiments/Bandit/train_gpt.py (shared, not duplicated)
...
echo "============================================"
echo "  BANDIT_II — crawler + X-WING oracle (LOOP_AWARE_GPTQ confirmed)"
echo "  Seed: ${SEED}"
echo "  inst_dim=32 FLOW | 4 flat + 1 crawler x 4 loops | DN=0"
echo "  EMA_START_STEP=4400 | EMA_DECAY=0.99 | LOOP_AWARE_GPTQ=1 (confirmed)"
echo "  NGRAM_EVAL_ORDER=9 | CUBRIC_CADENCE=32 | COMPLEMENT_ALPHA=0.5"
echo "  Shared n-gram tables | 3D Cubric 54-cell warm-start"
echo "  GPTQ: train data only, fires post-wallclock-cap (LEGAL)"
...
torchrun --standalone --nproc_per_node="${NPROC_PER_NODE}" \
    "${REPO_ROOT}/experiments/Bandit/train_gpt.py" \
    2>&1 | tee "logs/bandit_ii_s${SEED}_$(date +%Y%m%d_%H%M%S).log"

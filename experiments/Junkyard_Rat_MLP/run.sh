#!/bin/bash
set -euo pipefail
# JUNKYARD RAT MLP: JR-01 winner copied into a dedicated fused-MLP branch
# This workspace exists to isolate full MLP mega-fusion work from the live JR lane.
...
echo "============================================"
echo "  JUNKYARD RAT MLP — JR-01 Base For Fused MLP"
echo "  Seed: ${SEED}"
echo "  Loader mode: ${LOADER_MODE:-coprime}"
echo "  Base-first lane | no trigram | no n-gram eval"
echo "  JR-01 winner copied for full MLP mega-fusion work"
...
torchrun --standalone --nproc_per_node="${NPROC_PER_NODE}" \
    ...
    2>&1 | tee "logs/junkyard_rat_mlp_s${SEED}_$(date +%Y%m%d_%H%M%S).log"

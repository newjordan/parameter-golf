...
from kernels.vortex_fused import BLOCK_SIZE
import numpy as np
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, x0: Tensor) -> Tensor:
        ...
        # We need q, k, v and seq_lens
        bsz, seqlen, dim = x_in.shape
        q = self.attn.c_q(self.attn_norm(x_in)).reshape(bsz, seqlen, self.attn.num_heads, self.attn.head_dim).transpose(1, 2)
        k = self.attn.c_k(self.attn_norm(x_in)).reshape(bsz, seqlen, self.attn.num_kv_heads, self.attn.head_dim).transpose(1, 2)
        v = self.attn.c_v(self.attn_norm(x_in)).reshape(bsz, seqlen, self.attn.num_kv_heads, self.attn.head_dim).transpose(1, 2)
        q = F.rms_norm(q, (q.size(-1),))
        k = F.rms_norm(k, (k.size(-1),))
        cos, sin = self.attn.rotary(seqlen, x.device, q.dtype)
        q = apply_rotary_emb(q, cos, sin)
        k = apply_rotary_emb(k, cos, sin)
        ...
        row_ptr = torch.zeros(bsz, self.attn.num_heads, num_q_blocks + 1, device=x.device, dtype=torch.int32)
        for i in range(num_q_blocks):
            row_ptr[:, :, i+1] = i + 1

        col_idx = torch.arange(num_q_blocks, device=x.device, dtype=torch.int32).reshape(1, 1, -1).expand(bsz, self.attn.num_heads, -1).contiguous()
        seq_lens = torch.tensor([T] * bsz, device=x.device, dtype=torch.int32)

        if self.proj.weight.ndim == 2 and self.proj.weight.shape[0] == dim:
            # proj_weight is (dim, dim), we need (head_dim, head_dim)
            proj_weight_hd = self.proj.weight[:self.attn.head_dim, :self.attn.head_dim]
        else:
        ...
        # Reshape to (bsz, seqlen, dim)
        mixed = mixed.transpose(1, 2).contiguous().reshape(bsz, seqlen, dim)

        # Instead of adding, we'll project back. Original eager: x = x + scale * (mixed + perturb * sin(mixed))
        # Since mixed here already includes the perturb + sin(mixed) math, we just scale and add.
        # But notice: `mixed` comes from the kernel. The kernel does:

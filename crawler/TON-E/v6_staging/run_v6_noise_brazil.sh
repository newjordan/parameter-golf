#!/bin/bash
# v6 noise-probe SHORT run: FIRST_LAYER_NOISE=brazil, sigma=0.1.
# 1xGPU, 300 iters, dense train-log, one end val. Used by run_v6_parallel.sh.
# Probes multiplicative |x|^0.5-scaled Gaussian (the "brazil" mode).
...
# v6 brazil: multiplicative per-element |x|^0.5 * randn noise.
export FIRST_LAYER_NOISE="brazil"
export FIRST_LAYER_NOISE_SIGMA="${FIRST_LAYER_NOISE_SIGMA:-0.1}"

echo "Launching v6 SHORT: FIRST_LAYER_NOISE=brazil sigma=${FIRST_LAYER_NOISE_SIGMA} on GPU=${CUDA_VISIBLE_DEVICES:-unset} (${ITERATIONS} iters)"

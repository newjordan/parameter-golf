#!/bin/bash
# v5c: 11D + relu² MLP (mult=1) hybrid + 8k vocab + 4xH100.
# Tests whether adding eager MLP capacity atop the fused vortex attn helps.
# Requires ADD_MLP=1 flag (Block.forward adds MLP residual when set).
# mlp_mult=1 keeps model under 16MB int8 cap at 11D+8k.
...
export ADD_MLP="${ADD_MLP:-1}"        # activates MLP branch in Block.forward
...
echo "Launching v5c: 11D + MLP(mult=1) + 8k vocab + 4xH100 + ${ITERATIONS} iters"

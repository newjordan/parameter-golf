...
class Hyperparameters:
    ...
    # Nightcrawler folding: reuse the deepest encoder flat block(s) inside each crawler loop.
    linked_flat_recur_layers = int(os.environ.get("LINKED_FLAT_RECUR_LAYERS", "0"))
...
class CrawlerGPT(nn.Module):
    ...
    def __init__(
        ...
        linked_flat_recur_layers: int = 0,
    ):
        super().__init__()
        ...
        linked_flat_recur_layers = min(max(int(linked_flat_recur_layers), 0), self.flat_encoder_layers)
        if crawler_loops <= 1:
            linked_flat_recur_layers = 0
        self.linked_flat_recur_indices = (
            list(range(self.flat_encoder_layers - linked_flat_recur_layers, self.flat_encoder_layers))
            if linked_flat_recur_layers > 0
            else []
        )
        self.linked_flat_recur_prefixes = tuple(
            f"flat_blocks.{idx}." for idx in self.linked_flat_recur_indices
        )
    ...
    def _run_crawler(self, x: Tensor, x0: Tensor, input_ids: Tensor, ve_cache: dict,
        ...
        for loop in range(self.crawler_loops):
            ...
            # Reuse the deepest encoder flat block(s) inside the crawler loop so the
            # same loop battery shapes both the flat recurrent slice and crawler body.
            for flat_idx in self.linked_flat_recur_indices:
                x_loop = self.flat_blocks[flat_idx](x_loop, x0, cos_sin=lcs)
...
def build_model(args: Hyperparameters, device: torch.device) -> nn.Module:
    ...
    if args.use_crawler:
        model = CrawlerGPT(
            ...
            linked_flat_recur_layers=args.linked_flat_recur_layers,
        )
    else:
        model = GPT(
            vocab_size=args.vocab_size,
...
def gptq_calibrate_loop_aware(model: nn.Module, train_pattern: str, device: torch.device,
    ...
    """
    linked_flat_prefixes = tuple(getattr(model, "linked_flat_recur_prefixes", ()))
    CRAWLER_PREFIXES = ("crawler_blocks.", "delta_net.", "loop_inst") + linked_flat_prefixes

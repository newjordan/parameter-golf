#!/bin/bash
# v6a: 11D vortex-only + 8k vocab + 4xH100 + 2k iters + FIRST_LAYER_NOISE=flat.
# Probes whether isotropic Gaussian noise on the token embedding (sigma=0.1)
# acts as a regularizer on the vortex-only cap established in v5a.
...
# v6a-specific: flat Gaussian first-layer noise injection.
export FIRST_LAYER_NOISE="flat"
export FIRST_LAYER_NOISE_SIGMA="0.1"

echo "Launching v6a: 11D vortex-only + 8k vocab + 4xH100 + ${ITERATIONS} iters"
